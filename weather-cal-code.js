...
const weatherCal = {
  ...
  initialize(name, iCloudInUse) {
    ...
    this.widgetUrl = "https://raw.githubusercontent.com/angi47/weather-cal/refs/heads/main/weather-cal.js"
    this.now = new Date()
    this.data = {}
  ...
  async editSettings(codeFilename, gitHubUrl) {
    ...
    if (response == menu.other) {
      const otherOptions = ["Completely reset widget", "Exit"]
  ...
  // The API key is no longer needed, so this is a placeholder.
  async getWeatherApiPath() {
 ...
 * -------------------------------------------- */
  ...
  async setupEvents() {
    return await this.getEventsData()
  },

  // Get event data (alias for setupEvents)
  async getEventsData() {
  ...
  async setupSunrise() {
	...
	const sunPath = this.fm.joinPath(this.fm.libraryDirectory(), "weather-cal-sundata")
	let sunData = this.getCache(sunPath, 60, 1440)

	if (!sunData || sunData.cacheExpired) {
		try {
			const sunReq = `https://api.open-meteo.com/v1/forecast?latitude=${this.data.location.latitude}&longitude=${this.data.location.longitude}&daily=sunrise,sunset&timezone=auto`
    ...
    this.data.sun = {}
    this.data.sun.sunrise = sunData ? new Date(sunData.daily.sunrise[0]).getTime() : null
    this.data.sun.sunset = sunData ? new Date(sunData.daily.sunset[0]).getTime() : null
    this.data.sun.tomorrow = sunData ? new Date(sunData.daily.sunrise[1]).getTime() : null
  },
  ...
  async setupWeather() {
    ...
    // The new API does not require a locale.
    if (!weatherData || weatherData.cacheExpired) {
      try {
        const apiPath = await this.getWeatherApiPath()
        const weatherReq = `${apiPath}?latitude=${this.data.location.latitude}&longitude=${this.data.location.longitude}&hourly=temperature_2m,weathercode&daily=weathercode,temperature_2m_max,temperature_2m_min,precipitation_probability_max&current_weather=true&timezone=auto`
    ...
    // Function to map WMO codes to OpenWeather codes.
    function getWeatherCode(wmoCode) {
        if (wmoCode == 0) return 800;
        if (wmoCode == 1) return 801;
        if (wmoCode == 2) return 803;
        if (wmoCode == 3) return 804;
        if (wmoCode >= 45 && wmoCode <= 48) return 741;
        if (wmoCode >= 51 && wmoCode <= 55) return 501;
        if (wmoCode >= 56 && wmoCode <= 57) return 511;
        if (wmoCode >= 61 && wmoCode <= 65) return 502;
        if (wmoCode >= 66 && wmoCode <= 67) return 511;
        if (wmoCode >= 71 && wmoCode <= 75) return 601;
        if (wmoCode >= 77 && wmoCode <= 77) return 611;
        if (wmoCode >= 80 && wmoCode <= 82) return 503;
        if (wmoCode >= 85 && wmoCode <= 86) return 602;
        if (wmoCode >= 95 && wmoCode <= 99) return 201;
        return 800;
    }

    this.data.weather = {}
    this.data.weather.currentTemp = weatherData ? weatherData.current_weather.temperature : null
    this.data.weather.currentCondition = weatherData ? getWeatherCode(weatherData.current_weather.weathercode) : 100
    this.data.weather.currentDescription = weatherData ? weatherData.current_weather.weathercode.toString() : "--"
    this.data.weather.todayHigh = weatherData ? weatherData.daily.temperature_2m_max[0] : null
    this.data.weather.todayLow = weatherData ? weatherData.daily.temperature_2m_min[0] : null

    this.data.weather.forecast = []
    this.data.weather.hourly = []
    for (let i=0; i <= 7; i++) {
      this.data.weather.forecast[i] = weatherData ? ({High: weatherData.daily.temperature_2m_max[i], Low: weatherData.daily.temperature_2m_min[i], Condition: getWeatherCode(weatherData.daily.weathercode[i])}) : { High: null, Low: null, Condition: 100 }
      this.data.weather.hourly[i] = weatherData ? ({Temp: weatherData.hourly.temperature_2m[i], Condition: getWeatherCode(weatherData.hourly.weathercode[i])}) : { Temp: null, Condition: 100 }
    }

    this.data.weather.tomorrowRain = weatherData ? weatherData.daily.precipitation_probability_max[1] : null
    this.data.weather.nextHourRain = null // The new API does not provide this value.
  },
...
if (moduleName == Script.name()) {
  await (async () => {
    ...
    await weatherCal.runSetup(name, true, "Weather Cal code", "https://raw.githubusercontent.com/li6i/weather_cal_my_ver/refs/heads/main/weather-cal-code.js")
