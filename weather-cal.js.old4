...
const custom = {
  ...
  // Hent events direkte
  const now = new Date()
  const end = new Date(now.getTime() + (24 * 60 * 60 * 1000)) // 24 timer frem
  ...
  // Hvis der ER events, vis IKKE citatet (lad weather-cal vise events som normalt)
  if (hasEvents) {
    console.log("Der er events - viser ikke citat")
    return
  }
