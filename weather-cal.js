...
const custom = {
  ...
  // Tjek om der er events
  const now = new Date()
  const end = new Date(now.getTime() + (24 * 60 * 60 * 1000))
  
  let hasNonAllDayEvents = false
  ...
  if (hasNonAllDayEvents) {
    // Der er events - vis dem med den originale funktion
    // Vi skal kalde weather-cal's originale events funktion
    const code = importModule(codeFilename)
    await code.events(column)
  } else {
    // Ingen events - vis citat
