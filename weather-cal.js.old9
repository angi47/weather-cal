...
const custom = {
  ...
  // Hjælpefunktion til at tjekke events i de næste 36 timer
  async hasEventsNext36Hours() {
    try {
      const now = new Date()
      ...
      for (const calendar of calendars) {
        ...
        if (events && events.length > 0) {
          console.log(`Fandt ${events.length} event(s) i de næste 36 timer`)
          return true
        }
      }
      
      console.log("Ingen events i de næste 36 timer")
      return false
    } catch (e) {
      console.error("Fejl ved tjek af events:", e)
      return false
    }
  },

  async events(column) {
    // Tjek om der er events i de næste 36 timer
    const hasEvents = await custom.hasEventsNext36Hours()
